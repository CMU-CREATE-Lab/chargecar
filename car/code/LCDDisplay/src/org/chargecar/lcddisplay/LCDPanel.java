...
public final class LCDPanel implements CharacterDisplay {
    ...
    public void setCharacter(final int row, final int col, final char character, final boolean doAscii) {
        setCharacter(row, col, String.valueOf(character), doAscii);
    }

    public void setCharacter(final int row, final int col, final String character, final boolean doAscii) {
        if (isValidRow(row) && isValidColumn(col)) {
            final LCD lcd = LCDProxy.getInstance();
            lcd.setText(row, col, String.valueOf(character));
        }
