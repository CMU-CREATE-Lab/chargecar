...
public class ConsoleWriter implements Visualizer {
	...
	public void visualizeSummary(List<SimulationResults> results) {
		...
		for (int i = 1; i < results.size(); i++) {
			...
			System.out.println(results.get(i).getPolicyName() + " i^2, vs. baseline: "
					+ d.format(currentSquaredSums.get(i))+ " :: " + p.format(i2Percent));
			double rangePercent = baseChargeSpentSum / chargeSpentSums.get(i);
			System.out.println(results.get(i).getPolicyName()+ " charge spent, range vs. baseline: " 
				+  d.format(chargeSpentSums.get(i))+ " :: " + p.format(rangePercent));
		}
	}
