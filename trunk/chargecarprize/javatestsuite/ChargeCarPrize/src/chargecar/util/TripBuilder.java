...
public class TripBuilder {
	...
	private static void interpolatePoints(List<Calendar> times, List<Double> lats, List<Double> lons, List<Double> eles) {
	...
	private static void removeTunnels(List<Calendar> times, List<Double> lats,	List<Double> lons, List<Double> eles) {
		...
		for(int i=1;i<times.size();i++){
			if(lats.get(i).compareTo(lats.get(i-1)) == 0 &&
				...
				if(Haversine(lats.get(i-1),lons.get(i-1),lats.get(i), lons.get(i)) > 50){
