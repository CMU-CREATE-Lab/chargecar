...
public final class ToggleBrakeLightMenuItemAction extends TwoOptionMenuItemAction {
    private static final Logger LOG = Logger.getLogger(ToggleBrakeLightMenuItemAction.class);
    ...
    protected void executeOption1Action() {
        setBrakeLightEnabled(true);
    }

    protected void executeOption2Action() {
        setBrakeLightEnabled(false);
    }

    private void setBrakeLightEnabled(final boolean actionState) {
        final LCD lcd = LCDProxy.getInstance();
        if (lcd == null) {
            LOG.error("ToggleBrakeLightMenuItemAction.setBrakeLightEnabled(): lcd is null");
            return;
        }
