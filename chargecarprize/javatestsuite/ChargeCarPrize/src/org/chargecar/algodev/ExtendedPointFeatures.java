...
public class ExtendedPointFeatures extends PointFeatures {
    ...
    public ExtendedPointFeatures(double latitude, double longitude,
	    double elevation, double bearing, double planarDist, double acceleration,
	    double speed, double powerDemand, double speedHistVar, double accelHistVar, int periodMS, Calendar time) {
	super(latitude, longitude, elevation, bearing, planarDist, acceleration, speed,
	...
	super(pf.getLatitude(), pf.getLongitude(), pf.getElevation(), pf.getBearing(), pf.getPlanarDist(), pf.getAcceleration(), pf.getSpeed(), pf.getPowerDemand(), pf.getPeriodMS(), pf.getTime());
	this.speedHistVar = speedHistVar;
	this.accelHistVar = accelHistVar;
    }
